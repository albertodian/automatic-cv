...
class ProfileSaveResponse(BaseModel):
    """Response for profile persistence"""
    success: bool
    message: str
...
class GenerateAuthenticatedCVRequest(BaseModel):
    """Request to generate CV for authenticated user"""
    job_description: str = Field(..., description="Job description text")
    template: str = Field(default=settings.DEFAULT_TEMPLATE, description="Template type")
    model_name: str = Field(default=settings.DEFAULT_MODEL, description="LLM model to use")


class GenerateAuthenticatedCVFromURLRequest(BaseModel):
    """Request to generate CV from URL for authenticated user"""
    job_url: HttpUrl = Field(..., description="URL of the job posting")
    template: str = Field(default=settings.DEFAULT_TEMPLATE, description="Template type")
    model_name: str = Field(default=settings.DEFAULT_MODEL, description="LLM model to use")
...
# ============================================================================
# Save the complete information about the user in the DB
# ============================================================================


@app.post("/api/cv/save", response_model=ProfileSaveResponse, tags=["Save CV"])
async def save_profile(
    profile: ProfileData,
    current_user: Dict[str, Any] = Depends(get_current_user)
):
    """
    Persist a profile JSON for the authenticated user.
    """
    try:
        profile_dict = profile.model_dump()
        ProfileService.upsert_profile(
...
# ============================================================================
# CV Generation Endpoints (from profile + job description)
# ============================================================================
...
async def generate_cv(request: GenerateCVRequest):
    ...
    This endpoint:
    1. Takes your profile (JSON) and a job description
...
async def generate_cv_authenticated(
    request: GenerateAuthenticatedCVRequest,
    current_user: Dict[str, Any] = Depends(get_current_user)
):
        """
        ...
        try:
            ...
            job_info = extract_relevant_job_info(request.job_description, request.model_name)
...
# ============================================================================
# CV Generation Endpoints (from url + job description)
# ============================================================================
...
async def generate_cv_from_url(request: GenerateCVFromURLRequest):
    ...
    1. Loading user's profile from request
    2. Fetching the job description from URL
    3. Parsing the job requirements
    4. Using RAG to retrieve relevant content
    5. Generating an optimized CV (JSON)
...
async def generate_cv_authenticated_from_url(
    request: GenerateAuthenticatedCVFromURLRequest,
    current_user: Dict[str, Any] = Depends(get_current_user)
):
        """
